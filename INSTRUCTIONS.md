...
Sources/ArnesKit/
  ...
  ModelProfile.swift       # capability manifest + fuzzy search (ModelCatalog actor, GET /models); maxCompletionTokens (top_provider.max_completion_tokens, the /messages output ceiling; nil when the manifest doesn't say); supportsVision (T5: OpenRouter `architecture.input_modalities` contains `image`, else the legacy `modality` string's input side; LiteLLM `supports_vision`; `unknownModelId` and the cross-router default **false** — the one capability assumed *off* when unstated, because an image to a text model fails the whole request; `acceptsImages(_:)`); `ModelProfile: Codable` (the cache file's row; `ModelRow` stays the `--json` contract); ModelCatalog(… cache: ManifestCache?, cacheKey:) + `ManifestSource` (network · cache(fetchedAt) · staleCache(fetchedAt)) + `manifestSource`: loadIfNeeded serves a fresh cached copy without a fetch, else `refresh()` (fetch → install → store when non-empty; a failure with nothing served installs the stale copy as `.staleCache`), `profile(for:)` refetches **once per process** for an id a `.cache` copy doesn't know (a model newer than the copy), `refresh()` is `arnes models --refresh`; a catalog without a cache is byte-for-byte the pre-cache one; isDecisionModel (jev, 2026-09-20: `architecture.output_modalities` contains `decisions`, else the legacy `modality` string's output side — `emitsDecisions(_:)`; the **second** assumed-off capability beside vision, because a decisions request to a chat model fails the whole request; gateway-init parameter default false, `unknownModelId` false)
  ...
  ReasoningState.swift     # ReasoningDetails: the vocabulary for replayable reasoning state on `Message.reasoningDetails` — OpenRouter's `reasoning_details` entry shape (`reasoning.text` {text, signature}, `reasoning.encrypted` {data, id?}; `format` = the dialect tag: anthropic-claude-v1 / openai-responses-v1), builders text(_:signature:format:) / encrypted(data:id:format:), stripped(_:) (a history copy without the entries: the non-OpenRouter chat request, a model swap), FragmentMerger (chat stream fragments by index/id → whole entries); plaintextAfterTruncation(_:) preserves only a full sequence of unsigned plain chat text at output cutoff; omittingEmptyAssistantMessages(_:) removes empty stripped messages from the request view while stored history positions remain stable
  ...
  RunTimeBudget.swift      # shared monotonic run clock and bounded request-boundary notices; optional Configuration.timeBudget, caller-enforced deadline; maxResponseTokens caps main-loop responses through typed dialect fields
  ...
  EvalJevJudge.swift       # the decisions grader (jev, 2026-09-20): JevResult {score = fraction of expected questions whose expectation held (on the means), passed = score ≥ threshold ∧ !unknown, unknown (failed request / missing or misshapen answer — never a pass), notes (misses named, ≤ maxNotesChars), questions [{name, kind, mean (noul: P(yes) · score: expected level · choice: p of the expected-else-modal key), variance (unbiased n−1, nil once), passed? (nil = recorded only), choice (modal pick)}], repeats, scoreVariance (sample variance of the per-repeat score — the repeatability signal), costUSD (Σ usage.cost), tokens; .unknown(notes:costUSD:)}; JevJudge — state(task:evidence:) (the rubric judge's evidence as a typed JSON object {task, report, diff ≤ 30 000, check_passed, check_output ≤ 2 000} — same clips, never the transcript), questions(from:), bridgedQuestions (non-blank criteria → nouls keyed c1…cN in order), grade (N sequential service.decide; any thrown request → .unknown with the spend so far, never a thrown trial — the gateway-404 landing), gradeBridge (criterion met ⇔ mean P(yes) ≥ bridgeCut 0.5, folded into the existing RubricResult contract + the JevResult detail beside it), fold (the pure math a test pins), within/variance/modal (ties by name)/topKey/cost/tokens/miss
  EvalReport.swift         # X5, pure: EvalModelSummary (model × dialect: trials/passed/passRate, tasks, tasksWithAnyPass/tasksWithAllPass, trialsPerTask = k, costUSD, graderCostUSD, avgSteps, avgSeconds, errors), EvalRegression (task × model × dialect: previousPassRate? (nil = no baseline row in the window) / previousTrials, currentPassRate / currentTrials), EvalComparison {regressions, fixes, withoutBaseline}; EvalReport.summaries (sorted like EvalStats, model/dialect tie-broken), passAtK / passPowK (tasks with ≥ 1 pass / with every pass over tasks; nil when k ≤ 1), compare(history:current:window:) (Window .last(rows: 5) = the newest rows per key, .days(N) = the N days before the run's earliest startedAt; regression = ≥ 0.8 → < 0.5, fix = ≤ 0.2 → ≥ 0.5, keyed on suite/task/model/dialect, every list ordered task · model · dialect); EvalGate {minPass?, failOnRegression} → passes(summaries:regressions:) / exitCode 0 | 2 (failedExitCode); jev (2026-09-20): EvalModelSummary.jevGraded (rows with jevScore or jevQuestions) + avgJevVariance (mean of the rows' jevVariance, nil when none) — defaulted init params; JudgeAlignmentRow {suite, judge, secondJudge, trials (both verdicts), decided (neither unknown), agreements, agreementRate (nil at 0 decided), meanAbsScoreDelta, primaryUnknowns/secondUnknowns, meanJevVariance} + JudgeAlignment.compute(_:) (pure, grouped suite × judge pair, ordered suite · judge · second)
...
Sources/arnes/             # the CLI
  ...
  DecideCommand.swift      # `arnes decide [state] --questions <file|json> [--state-json] [--json] [-m <slug>]` — typed decisions over OpenRouter's `POST /api/alpha/decisions` (alpha; a LiteLLM/OpenAI-compatible gateway 404s): System One models (default `typesafe/jev-1.13`) answer noul (yes/no probability) / choice (picked key + per-option probabilities + confidence) / score (fractional expected level + index-keyed probabilities + legend) questions about a state with calibrated probabilities, no generated text — routing/ranking/verification material, wired through OpenRouterSwift 0.3.0's `OpenRouterService.decide(_:)` (`DecisionRequest`/`DecisionQuestion`/`DecisionResponse`/`DecisionAnswer`, invariant 5). Pure pieces: `loadQuestions` (inline JSON when the trimmed flag starts `{`, else a path — absolute or against the cwd), `parseState` (plain string, or JSON under --state-json), `lines(for:requested:)` (the text view: `decision from <post-routing model> (<provider>):`, answers sorted by name — choice options highest-probability first, score levels in rubric order labeled through the legend —, the `[$cost · N in / M out]` footer), `answerText`, `summary(of:)` (`name=answer` pairs, capped 200). State from the argument or stdin (`-`/omitted). Invariant 4: every call appends a RunRecord — dialect `decisions`, packFamily `ModelFamily(modelId:)`, provider `runtime.traits.name`, steps 1, the post-routing `response.model` in routedModels, usage cost/tokens, the summary; a failed call records finished=false with the error text as summary. `--json` prints `DecisionDocument` (JSONOutput.swift); REPL twin (2026-09-20): `defaultModel` (`typesafe/jev-1.13`), `makeRecord(state:model:provider:)`/`finish(_:with:)` (the shared invariant-4 record halves — built before the request, filled on the answer) and `notice(state:response:requested:)` (what `/decide` queues through `Session.notify`: the framing line, the state ≤ 300, and the *same* `lines(for:requested:)` render the UI printed — agent and user read one answer)
...
- [x] Add opt-in time-budget experiments (2026-09-08) — `do --time-aware --timeout`
      gives the model a shared monotonic clock through bounded user-history notices;
      delegated work keeps the parent's deadline and the system/tool prefix stays stable.
      `--max-response-tokens` caps main-loop responses through typed chat/messages/responses
      fields, narrowed by the manifest ceiling. Chat spelling follows manifest capability
      or provider traits independently of the reasoning dial. Existing truncation recovery
      never dispatches incomplete calls and stops after its bounded continuation allowance.
      Both controls default off/unset; the caller still enforces the actual timeout.
      Harbor records the flags in schema-4 provenance and checks binary support at install.
      Add a separate early-implementation prompt proposal and controlled protocol under
      evals/ab; no default pack changes or model-quality gains are claimed. Validation:
      1,905 Swift tests, one skipped, zero failures; 29 offline Python checks; five isolated
      Linux CLI/adapter cases, including interruption of a held response at a two-second
      timeout; three Harbor 0.16.1 installation checks. These use zero external model calls.
      Real benchmark comparisons remain human-operated.
- [x] Preserve plaintext reasoning through output cutoffs (2026-09-09) — the time-cap
      development arm passed 1/6 attempts versus control's 2/6; four capped trials ended
      after two reasoning-only cutoffs without required artifacts. Record the failed
      experiment under evals/ab and keep the cap unset by default. A chat cutoff without
      a complete call now retains the full sequence of unsigned `reasoning.text` entries
      (format absent or `unknown`) when the provider replays reasoning. Mixed, signed,
      opaque and native cutoff state retain the existing behavior. The single continuation
      allowance, explicit response cap, cost/time limits and incomplete-call exclusion
      remain intact. A model swap preserves history indices; empty stripped assistant
      messages are omitted only from the chat request view. Validation: 1,911 Swift tests,
      one skipped, zero failures; 29 Python adapter checks; seven isolated Linux CLI cases
...
- [x] `arnes decide` — typed decisions over OpenRouter's Decisions API (2026-09-19). System One
      models (`typesafe/jev-1.13`, released 2026-09-18) generate no text: `POST
      /api/alpha/decisions` takes a state (string/object/array) plus typed questions and returns
      calibrated probabilities — `noul` (yes/no probability), `choice` (picked key + per-option
      probabilities + confidence), `score` (fractional expected level + index-keyed
      probabilities + legend). Chat endpoints cannot reach these models, so the wire went
      upstream per invariant 5: OpenRouterSwift 0.3.0 adds `DecisionRequest`/`DecisionQuestion`
      (criteria as `{key: description}` or an ordered level array, both wire spellings)/
      `DecisionResponse`/`DecisionAnswer` (score probabilities/legend decode object *and* array
      forms into the index-keyed shape; `scoreDistribution`/`scoreLabel` helpers) and
      `OpenRouterService.decide(_:)` (Package.swift pin bumped to `from: "0.3.0"`; the response
      shape was pinned against a live jev-1.13 call, kept verbatim as the decoding fixture in
      both repos). `DecideCommand.swift` is the CLI: state from the argument or stdin
      (`--state-json` for object/array states), `--questions <file|inline JSON>`, text view
      (answers sorted by name, choice options by probability, score levels labeled through the
      legend) or a `--json` `DecisionDocument` (JSONOutput.swift; `requested` = the slug asked,
      `model` = post-routing). Every call appends a RunRecord — invariant 4 — with dialect
      `decisions`, packFamily from `ModelFamily(modelId:)`, the post-routing model in
      `routedModels`, usage cost/tokens, and a `name=answer` summary; a failed call records the
      unfinished row with the error as summary. OpenRouter-only (alpha path, not `/api/v1`);
      a gateway 404s and the error surfaces. SKILL.md gained the "Typed decisions" section in
      the same commit. (7 CLI tests over the pure pieces + 3 upstream tests; live smoke is the
      human's step: `arnes decide "Payouts failing for 3 days" --questions '{"urgent":
      {"type": "noul", "instructions": "Is this urgent?"}}'`.)
- [x] `/decide` — the REPL twin of `arnes decide` (2026-09-20). The user wanted decisions from
      interactive mode with the whole jev answer visible whatever the agent gets; a tool was
      rejected (invariant 3, and the tool list is a prompt-cache prefix — a `decide` tool would
      ride every request of every session), a slash command touches only the CLI layer.
      `SlashCommand.decide(argument:)` + `decideArguments` (inline questions JSON brace-matched
      via the JSON-string-aware `matchingBrace`, or a file path first token; the rest is the
      state) + the `expandingPastes` arm (a pasted questions object expands at the door like
      `/schema`'s). `Interactive.handleDecide` calls `ReplDials.service` (new, defaulted-nil
      field) on `Decide.defaultModel`, prints the full `Decide.lines` render in the UI
      unconditionally, appends the invariant-4 RunRecord through the halves now shared with the
      headless path (`Decide.makeRecord`/`finish`), and hands the agent the *same* render via
      `Session.notify` (`Decide.notice` — the sanctioned user-message channel; the system prompt
      and tool list never move, `PrefixStabilityTests` untouched) with the dim `— the model sees
      this decision with your next message` tail; a failed call records the unfinished row and
      prints red. `/decide` joins helpText, the autocomplete builtins (after `/verify`), and
...
- [x] Jev as an eval grader (2026-09-20). The user wanted a jev-based eval framework inside
      the existing pipeline (LangChain's Deep Agents experiment: a decisions judge at
      ~$0.0004/call with 92–913× lower score variance than LLM judges). Four pieces, all in
      the pipeline `RubricJudge` already lives in — never a separate subsystem:
      (1) `EvalTask.Jev` — typed noul/choice/score questions (the wire's own
      `DecisionQuestion.Kind`/`Criteria`, the same grammar `arnes decide --questions` takes)
      with per-question `expect` {min, max, choice} checked **against the mean across
      repeats**, `gate` (default true), `repeats` 1–9, `threshold` (fraction of expected
      questions, default 1.0); `jevProblem` refuses pre-flight like `rubricProblem` (empty
      questions, a gating block with no expectations, score levels outside 2–10, a bad
      expect, repeats/threshold out of range). `JevJudge` (`EvalJevJudge.swift`) sends the
      rubric judge's evidence as a typed state object (same clip caps, never the transcript)
      through `service.decide` — no prompt, no schema, no retry: a failed or malformed reply
      is `.unknown`, never a thrown trial (also where a gateway's decisions 404 lands);
      `fold` is the pure math a test pins (means, n−1 variances, per-repeat score variance —
      the repeatability signal).
      (2) **The bridge**: a `--judge` whose manifest profile is a decisions model grades
      plain `rubric.criteria` as noul questions keyed `c1…cN` (met ⇔ mean P(yes) ≥
      `bridgeCut` 0.5) folded into the *existing* rubric contract — scoreboard, compare,
      gate, progress all unchanged; per-criterion means kept in `jevQuestions` for audit.
      Detection is a manifest bit per invariant 1: `ModelProfile.isDecisionModel`
      (`architecture.output_modalities` contains `decisions`, verified live on
      typesafe/jev-1.13; legacy `modality` output side as fallback) — the **second**
      documented assumed-off capability beside vision, since a decisions request to a chat
      model fails whole. The `jev` block itself never gates on the profile (the block is the
      opt-in; the ladder is `jev.model` > `--judge` when decisions-capable > the runner's
      `decisionJudge`, which the CLI feeds `Decide.defaultModel` — ArnesKit hardcodes no
      slug), so an unlisted jev slug still works and only the bridge needs the manifest.
      (3) **Repeats + alignment**: `--judge-repeats` N (1–9; `jev.repeats` wins; validate()
      refuses the rest) re-asks a decisions judge and lands `jevRepeats`/`jevVariance` on
      the row; `--second-judge <model>` grades every rubric task's evidence twice (routed by
      *its* profile — decisions → the bridge fold, chat → `RubricJudge`), recorded in
      never-gating `second*` fields, `Eval.alignmentLines` prints the agreement block after
      the run, and the new `arnes evals judges [--suite --label --json]` aggregates
      suite × judge-pair agreement/|Δscore|/unknowns/variance across the whole history
      (`JudgeAlignment.compute`, pure in EvalReport.swift).
      (4) Rows and views: `EvalOutcome` gains `judgeModel`, `jevScore/Passed/Unknown/Notes`,
      `jevRepeats`/`jevVariance` (written only under repeats), `jevQuestions`
      (`JevQuestionRecord` {name, kind string, mean, variance, passed, choice}), and the
      `second*` four — all decodeIfPresent with an old-row test; `gradedVerdict` gains a
      5-arg overload (gating jev unknown ⇒ fail; the 4-arg spelling delegates);
      `EvalModelSummary.jevGraded`/`avgJevVariance` (defaulted init params);
      `progressLine` appends ` · jev 1.00 ✓ (σ² …, n=…)` / ` · judge² ✓` only when present
      (the pinned ungraded line is byte-identical), `graderCostLine` says `(rubric + jev)`
      only when a decisions judge graded, `jevLines` adds `  jev N graded · σ² …` under a
      model's table row; `EvalOutcomeRow`/`EvalModelRow` grow the snake_case twins +
      `EvalJevQuestionRow`, and `EvalJudgesDocument {type: eval_judges}` is new — the
      one golden that moved is `testEvalReportDocumentKeysAndNulls` (additive keys).
      Jev judge calls append **no RunRecord** — grader precedent (RubricJudge/Verifier):
      spend books to `graderCostUSD` so model comparisons stay fair and `arnes runs` isn't
      double-counted; `arnes decide` keeps appending (a user-invoked path, invariant 4).
      Mock: scriptable `decide` (`decisionResponses`/`decisionErrors`/`decisionRequests`)
      + `Fixtures.decisionResponse`/`noulDecision`/`decisionsManifestModel`/
      `liveDecisionFixture` (the DecideCommandTests capture, now shared). Suite:
      `evals/jev/` (a three-question-type gated task, a bridge demo, a `repeats: 3`
      variance task with a recorded-only question) + README; `evals/graded` untouched
      (A/B-stable). Docs: SKILL.md eval section + Typed decisions cross-ref,
      `.arnes/skills/jev/SKILL.md` cross-ref, AGENTS.md invariant-1 second exception +
      layout. Live smoke (human): `arnes models | grep -i jev` (whether GET /models lists
      it — only the bridge cares), `arnes eval evals/jev -m <cheap> --judge-repeats 3`,
      `arnes eval evals/graded --task add-docstring-rubric -m <cheap> --judge <llm>
      --second-judge typesafe/jev-1.13`, `arnes evals judges`, then `arnes evals prune`.
      (24 new tests: EvalJevJudgeTests — decode/old-row, the fold table, state clipping,
      bridged questions, jevProblem, gradedVerdict, five end-to-end trials over the mock's
      scriptable decide, the live-fixture fold; ModelProfileTests ×3; EvalReportTests ×2;
      CLI flag/line/document tests; suite 1975 green, the ungraded progress line and stats
      table pinned untouched.)
      Follow-up (same day): user-facing docs. README gains the jev lines in the eval
      command block, the `jev` key (with a worked JSON example) in the graders paragraph —
      now "four optional keys" —, a new `## Typed decisions (jev)` section (the first
      README coverage of `arnes decide`/`/decide` at all, with the live-fixture render) in
      the top nav, and the feature-list bullet; AGENTS.md `## Working on it` gains a
      one-bullet pointer (cheap graded reruns, the bridge, repeats, `evals judges`,
      `evals/jev/README.md` as the worked example) — 17.6 KB, under the 32 KB cap.
