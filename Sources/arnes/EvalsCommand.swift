...
struct EvalsCapture: AsyncParsableCommand {
  ...
  func run() async throws {
    ...
    if let task {
      ...
      let loaded = try store.load(id: meta.id)
    ...
    var file = directory.appendingPathComponent("\(result.task.id).json")
    var suffix = 2
    while FileManager.default.fileExists(atPath: file.path) {
      file = directory.appendingPathComponent("\(result.task.id)-\(suffix).json")
      suffix += 1
    }
    let encoder = JSONEncoder()
    encoder.outputFormatting = [.prettyPrinted, .sortedKeys, .withoutEscapingSlashes]
    try encoder.encode(result.task).write(to: file)
